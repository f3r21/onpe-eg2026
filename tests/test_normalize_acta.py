...
def test_normalize_mesa_inexistente_devuelve_none(
...
def test_normalize_acta_e_tiene_detalle_pero_totales_null(
...
def test_normalize_ubigeo_distrito_en_todas_las_filas_hijas(
    acta_c: dict[str, Any], snapshot_ts_ms: int
):
    """Todas las filas (votos/linea/archivos) heredan ubigeoDistrito del parent."""
    acta_c["archivos"] = [
        {"id": "a1", "tipo": 1, "nombre": "x", "descripcion": "y", "daudFechaCreacion": 0}
    ]
    acta_c["lineaTiempo"] = [
    ...
    assert all(ev["ubigeoDistrito"] == "040102" for ev in linea)
